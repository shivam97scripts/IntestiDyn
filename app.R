...
server <- function(input, output, session) {
  ...
  output$splinePlot <- renderPlot({
    ...
    ggplot() +
      ...
      annotate("text", x = 6, y = 2.7, label = "Inflammation gene expression", color = "red", hjust = 1,fontface="bold", size=10) +
      annotate("text", x = 6, y = 2.1, label = unique(plot_data$label), color = "darkgreen", hjust = 1,fontface="bold",size=10)
  })
